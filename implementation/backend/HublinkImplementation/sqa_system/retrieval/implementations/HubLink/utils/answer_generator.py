...
class AnswerGenerator:
    ...
    def get_final_answer(self,
        ...
        _ph = ProgressHandler()
        _ph.add_task(
            string_id="final_answer_generation",
        ...
        # Step 1 complete: prompt prepared
        ...
        # Add the source information to the final answer and track which hubs are cited
        referenced_hub_answers = []
        response += "\n\n"
        for index, hub_answer in enumerate(hub_answers):
            if f"[{index+1}]" in response:
                ...
                referenced_hub_answers.append(hub_answer)
            else:
                logger.debug(
                    "The LLM did not include the source %s in the final answer.",
                    hub_answer.source_identifier)

        # Use only actually cited hubs for graph nodes; fall back to all if none cited
        hubs_for_context = referenced_hub_answers if referenced_hub_answers else hub_answers
        ...
        # Step 3 complete: contexts extracted
        _ph.finish_by_string_id("final_answer_generation")
        return RetrievalAnswer(contexts=contexts, retriever_answer=response, sources=referenced_sources)
    ...
    def _get_relevant_triples_from_hubs(self,
        ...
        # Collect sorted triples per hub first
        hub_triples_list: List[List[Tuple[Triple, float]]] = []
        for hub_answer in hub_answers:
            ...
            hub_triples_list.append(hub_triples)

        if filter_output_triples:
            # Filter all hubs in parallel – each hub's LLM call is independent
            def _filter_hub(index: int) -> List[Tuple[Triple, float]]:
                hub_triples = hub_triples_list[index]
                hub_answer = hub_answers[index]
                filtered = self._filter_triple_contexts_by_llm(
                    hub_triples, question, hub_answer.hub_answer, [])
                return hub_triples if filtered is None else filtered

            results: List[List[Tuple[Triple, float]]] = [None] * len(hub_answers)
            with ThreadPoolExecutor(max_workers=len(hub_answers) or 1) as executor:
                futures = {
                    executor.submit(_filter_hub, i): i
                    for i in range(len(hub_answers))
                }
                for future in as_completed(futures):
                    i = futures[future]
                    results[i] = future.result()
                    logger.info(
                        "Triple filter: hub %d/%d done",
                        sum(r is not None for r in results),
                        len(hub_answers),
                    )
            for triples in results:
                all_triples_with_score.extend(triples)
        else:
            for hub_triples in hub_triples_list:
